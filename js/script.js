...
const SUPABASE_URL = 'https://pfllreeqtdmbrbvgnusg.supabase.co'; // Misalnya: https://your-project.supabase.co
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBmbGxyZWVxdGRtYnJidmdudXNnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMwNTIzNzUsImV4cCI6MjA3ODYyODM3NX0.LUtOubmLAyHavdONvozfTSHnXT1_hiCAXjY0RQIoGvw'; // Anon public key dari Supabase
...
// Fungsi untuk load editor carousel di admin (dengan upload)
async function loadCarouselEditor() {
    ...
    carouselData.forEach((slide, index) => {
        ...
        form.innerHTML = `
            ...
            <input type="file" accept="image/*" data-index="${index}" data-field="image-file">
            <input type="text" value="${slide.image}" placeholder="URL Gambar (atau upload di atas)" data-index="${index}" data-field="image">
...
// Fungsi untuk simpan carousel (dengan upload)
async function saveCarousel() {
    ...
    for (let i = 0; i < forms.length; i++) {
        ...
        const imageFile = forms[i].querySelector('[data-field="image-file"]').files[0];
        let image = forms[i].querySelector('[data-field="image"]').value;

        if (imageFile) {
            image = await uploadImage(imageFile);
            if (!image) continue; // Skip jika upload gagal
        }

        const carouselData = await fetchCarouselData();
        const id = carouselData[i].id;
        const { error } = await supabaseClient.from('carousel').update({ title, description, image }).eq('id', id);
        if (error) alert('Error saving carousel: ' + error.message);
    }
...
async function saveNews() {
    ...
    for (let i = 0; i < forms.length; i++) {
        ...
        const { error } = await supabaseClient.from('news').update({ title, summary, image }).eq('id', id);
        if (error) alert('Error saving news: ' + error.message);
    }
    alert('Berita disimpan!');
    loadNewsEditor();
}

// Fungsi untuk tambah slide carousel (dengan upload)
async function addCarouselSlide() {
    try {
        const { error } = await supabaseClient.from('carousel').insert([{ title: 'Judul Baru', description: 'Deskripsi Baru', image: 'images/default.jpg' }]);
        if (error) throw error;
        alert('Slide carousel ditambahkan!');
        loadCarouselEditor();
    } catch (error) {
        alert('Error menambah slide: ' + error.message);
    }
}
...
async function addNewsItem() {
    try {
        const { error } = await supabaseClient.from('news').insert([{ title: 'Judul Baru', summary: 'Summary Baru', image: 'images/default.jpg' }]);
        if (error) throw error;
        alert('Berita ditambahkan!');
        loadNewsEditor();
    } catch (error) {
        alert('Error menambah berita: ' + error.message);
    }
}
